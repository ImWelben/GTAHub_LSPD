...
const ticketSchema = require(`${process.cwd()}/modelos/tickets-metro`);
...
module.exports = client => {
    ...
    client.on("interactionCreate", async interaction => {
        try {
            ...
            //comprobaciones previas
            if (!setup || !setup.sistema_tickets_metro || !setup.sistema_tickets_metro.canal || interaction.channelId !== setup.sistema_tickets_metro.canal || interaction.message.id !== setup.sistema_tickets_metro.mensaje) return;
            ...
            channel.send({
                embeds: [new Discord.EmbedBuilder()
                    .setTitle(`Recepción de Correos`)
                    .setAuthor({ name: 'Metropolitan Division', iconURL: 'https://imgur.com/Z3hVJta.png'})
                    .setDescription(`Buen día <@${interaction.member.user.id}>. En breves un miembro de cúpula se pondrá en contacto con usted, describa el motivo de la apertura del ticket.`)
                    .setColor("#5865F2")
                    .setFooter({ text: 'Powered By Welben', iconURL: 'https://imgur.com/pC8g5Rz.png' })
                ],
        ...
        try {
            ...
            switch (interaction.customId) {
                ...
                case "guardar_ticket": {
                    ...
                    client.channels.cache.get(`1017808832434536458`).send({
                        embeds: [new Discord.EmbedBuilder()
                            .setTitle(`Registro de ticket`)
                            .setAuthor({ name: 'Metropolitan Division', iconURL: 'https://imgur.com/Z3hVJta.png'})
                            .addFields(
                                { name:`Nombre del ticket`, value: `${interaction.channel.name}`},
                                { name:"Fecha", value: `<t:${Math.round(Date.now() / 1000)}>`},
                            )
                            .setThumbnail('https://imgur.com/Z3hVJta.png')
                            .setColor("#0f2940")
