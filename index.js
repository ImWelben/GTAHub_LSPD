...
client.on("ready", () => {
    console.log(`Presencia creada`.brightRed);
    client.user.setPresence({ activities: [{ name: 'By Welben' }], status: 'online' }); 
...
//SISTEMA DE BIENVENIDA - LSPD GTAHUB
/*
const channelId = '808783338025910312' // welcome channel

  client.on('guildMemberAdd', (member) => {
    const channel = member.guild.channels.cache.get(channelId)
    channel.send({embeds: [new Discord.EmbedBuilder()
        .setTitle(`:mobile_phone: NUEVO REGISTRO :mobile_phone:`)
        .setThumbnail('https://imgur.com/hJJpZo7.png.png')
        .setDescription(`¡Bienvenido/a <@${member.id}>!\nRecuerda **colocarte tu Nombre y Apellido**\n\nTe damos la bienvenida a la aplicación oficial de la LSPD.\nComo primera instancia por favor revisa <#831981634398978058> & <#836592679030226974> para estar actualizado sobre toda la informacion entorno a nuestra aplicación.`)
        .setImage(`https://imgur.com/I1muY9o.png`)
        .setColor("#0E355A")
        ...
        if(data) {
            if(member.guild.channels.cache.get(data.welcomechannel)){
                ...
                channel.send({embeds: [new Discord.EmbedBuilder()
                    ...
                    .setDescription(data.welcomemessage.replace(/{usuario}/, member))
                    .setImage(attachment)
                    .setColor("#064073")
                    .setFooter({ text: 'Powered By Welben', iconURL: 'https://imgur.com/pC8g5Rz.png' })
                ],
                    content: data.welcomemessage.replace(/{usuario}/, member), files: [attachment]})
            }
        }
